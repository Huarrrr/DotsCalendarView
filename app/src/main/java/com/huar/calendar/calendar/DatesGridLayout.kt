...
class DatesGridLayout : ViewGroup, DateText.DateSelectListener {
    ...
    private fun getAttributeValues() {
        ...
        val attributes = mContext.theme.obtainStyledAttributes(mAttrs,
            R.styleable.DatesGridLayout,
            mDefStyleAttr,
            mDefStyleRes)
        try {
            ...
            sWeekStartDay =
                attributes.getInt(R.styleable.DatesGridLayout_weekStartDay, Calendar.MONDAY)
    ...
    private fun setCalendarProperties() {
        ...
        mNoOfWeeks =
            if (showOnlyCurrentMonthWeeks) ceil(((mNoOfCurrentMonthDays + mMonthStartDayOffset).toFloat() / 7.0f).toDouble()).toInt() else 6
        mTotalNoOfDays = 7 * mNoOfWeeks
        mTranslationDistance =
            (mSelectedWeekNo - 1) * mContext.resources.getDimension(R.dimen.dimen_date_text_view)
...
//                    isDisabled =
//                        c[Calendar.MONTH] == mCurrentCalendar[Calendar.MONTH] && c[Calendar.YEAR] == mCurrentCalendar[Calendar.YEAR] && c[Calendar.DAY_OF_MONTH] == mCurrentCalendar[Calendar.DAY_OF_MONTH]
//                }
...
//            }
            ...
            dateTextView.setProperties(isCurrentMonth,
                hasEvent,
                isSelected,
                EventsCalendarUtil.isToday(mCurrentCalendar),
                mCurrentCalendar,
                false, //全部不设置为非过去日历，当天之前的也可以点击
                isDisabled)

            if (!Events.isWithinMonthSpan(mCurrentCalendar)) dateTextView.isClickable = false

            if (EventsCalendarUtil.areDatesSame(
    ...
    private inner class RefreshDotsTask : AsyncTask<Int, Int, Boolean>() {
        ...
        private fun fillDotsModificationArrays() {
            ...
            for (i in 1..mTotalNoOfDays) {
                ...
                val dateTextView = getChildAt(i - 1) as DateText
                if (dotsData != null) {
                    if (dateTextView.hasEvent && !dotsData.hasEvent(mCurrentCalendar.get(Calendar.DATE))) mDotsRemovalArray.add(
                        dateTextView)
                    else if (!dateTextView.hasEvent && dotsData.hasEvent(mCurrentCalendar.get(
                            Calendar.DATE))
...
//        val heightConstraints: Int = paddingTop + paddingBottom
        val dateWidthSpec: Int =
            MeasureSpec.makeMeasureSpec(mDateTextWidth.toInt(), MeasureSpec.EXACTLY)
        val dateHeightSpec: Int =
            MeasureSpec.makeMeasureSpec(mDateTextHeight.toInt(), MeasureSpec.EXACTLY)
        ...
        mDateTextHeight =
            if (MeasureSpec.getMode(heightMeasureSpec) == MeasureSpec.EXACTLY) (MeasureSpec.getSize(
                heightMeasureSpec)
                .toFloat() - resources.getDimension(R.dimen.dimen_date_text_view) / 1.5f - resources.getDimension(
                R.dimen.height_week_day_header)) / mNoOfWeeks
    ...
    override fun onLayout(changed: Boolean, l: Int, t: Int, r: Int, b: Int) {
        ...
        for (i in 0 until mNoOfWeeks) {
            ...
            for (j in 0..6) {
                ...
                child.layout(dateLeft.toInt(),
                    weekTopOffset.toInt(),
                    (dateLeft + mDateTextWidth).toInt(),
                    (weekTopOffset + mDateTextHeight).toInt())
    ...
    override fun onDateTextViewSelected(dateText: DateText, isClick: Boolean) {
        if (selectedDateText != null && selectedDateText != dateText) selectedDateText?.unSelect(
            isClick)
        ...
        post {
            ...
            if (EventsCalendarUtil.currentMode == EventsCalendarUtil.WEEK_MODE) translationY =
                -mTranslationDistance
        }
    }
    ...
    fun selectDefaultDate(defaultDate: Int) {
        ...
        if (EventsCalendarUtil.currentMode == EventsCalendarUtil.MONTH_MODE) {
            if (defaultDate < 29) (getChildAt(mMonthStartDayOffset - 1 + defaultDate) as DateText).select(
                false)
            else {
    ...
    fun selectDate(date: Calendar) {
        val selectedDate =
            getChildAt(mMonthStartDayOffset - 1 + date.get(Calendar.DATE)) as DateText
        selectedDate.select(true)
    }

    fun setShowOnlyCurrentMonthWeeks(showOnlyCurrentMonthWeeks: Boolean) {
        Companion.showOnlyCurrentMonthWeeks = showOnlyCurrentMonthWeeks
    }
