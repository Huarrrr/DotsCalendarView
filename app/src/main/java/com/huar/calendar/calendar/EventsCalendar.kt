...
class EventsCalendar : ViewPager, MonthView.Callback {
    ...
    private fun init(context: Context, attrs: AttributeSet?) {
        ...
        try {
            ...
            EventsCalendarUtil.secondaryTextColor =
                attributes.getColor(R.styleable.EventsCalendar_secondaryTextColor,
            ...
            EventsCalendarUtil.selectionColor =
                attributes.getColor(R.styleable.EventsCalendar_selectionColor,
                    EventsCalendarUtil.primaryTextColor)
            EventsCalendarUtil.rangeSelectionColor =
                attributes.getColor(R.styleable.EventsCalendar_rangeSelectionColor,
                    EventsCalendarUtil.primaryTextColor)
            EventsCalendarUtil.rangeSelectionStartColor =
                attributes.getColor(R.styleable.EventsCalendar_rangeSelectionStartColor,
                    EventsCalendarUtil.primaryTextColor)
            EventsCalendarUtil.rangeSelectionEndColor =
                attributes.getColor(R.styleable.EventsCalendar_rangeSelectionEndColor,
                    EventsCalendarUtil.primaryTextColor)
            EventsCalendarUtil.eventDotColor =
                attributes.getColor(R.styleable.EventsCalendar_eventDotColor,
                    EventsCalendarUtil.eventDotColor)
            EventsCalendarUtil.monthTitleColor =
                attributes.getColor(R.styleable.EventsCalendar_monthTitleColor,
                    EventsCalendarUtil.secondaryTextColor)
            EventsCalendarUtil.weekHeaderColor =
                attributes.getColor(R.styleable.EventsCalendar_weekHeaderColor,
                    EventsCalendarUtil.secondaryTextColor)
            EventsCalendarUtil.isBoldTextOnSelectionEnabled =
                attributes.getBoolean(R.styleable.EventsCalendar_isBoldTextOnSelectionEnabled,
                    false)
            EventsCalendarUtil.dateTextFontSize =
                EventsCalendarUtil.convertPixelsToDp(attributes.getDimension(R.styleable.EventsCalendar_datesTextSize,
                    mContext.resources.getDimension(R.dimen.text_calendar_date)), mContext)
            EventsCalendarUtil.weekHeaderFontSize =
                EventsCalendarUtil.convertPixelsToDp(attributes.getDimension(R.styleable.EventsCalendar_weekHeaderTextSize,
                    mContext.resources.getDimension(R.dimen.text_week_day_header)), mContext)
            EventsCalendarUtil.monthTitleFontSize =
                EventsCalendarUtil.convertPixelsToDp(attributes.getDimension(R.styleable.EventsCalendar_monthTitleTextSize,
                    mContext.resources.getDimension(R.dimen.text_month_title)), mContext)
        } finally {
            attributes.recycle()
        }
    ...
    fun build() {
        ...
        mSelectedMonthPosition =
            EventsCalendarUtil.getMonthPositionForDay(EventsCalendarUtil.getCurrentSelectedDate(),
                startMonth)
        mSelectedWeekPosition =
            EventsCalendarUtil.getWeekPosition(EventsCalendarUtil.getCurrentSelectedDate(),
                startMonth)
...
//            e.printStackTrace()
    ...
    override fun onDaySelected(isClick: Boolean) {
        if (mCallback != null) {
            if (!DatesGridLayout.selectedDateText?.isCurrentMonth!!) {
                ...
                if (itemNo >= 0 && itemNo <= EventsCalendarUtil.getWeekCount(mMinMonth,
                        mMaxMonth)
                ) {
                ...
                mCallback?.onMonthChanged(EventsCalendarUtil.getCurrentSelectedDate()) //非当月也要回调月份切换，因为title和日历分开
            }
        }
    }
    ...
    fun addEvent(date: String) {
        Events.add(EventsCalendarUtil.getAssignCalendar(date,null))
    }
    ...
    fun addEvent(arrayOfCalendarStr: Array<String>) {
        for (str in arrayOfCalendarStr) {
            Events.add(EventsCalendarUtil.getAssignCalendar(str,null))
        }
    }
