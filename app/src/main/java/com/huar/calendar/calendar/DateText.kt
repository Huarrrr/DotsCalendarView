...
class DateText : View {
    ...
    private fun init(context: Context, attrs: AttributeSet?, defStyleAttr: Int, defStyleRes: Int) {
        ...
        if (doInitializeStaticVariables) {
            ...
            mTodayPaint = Paint(Paint.ANTI_ALIAS_FLAG).apply {
                color = EventsCalendarUtil.eventDotColor
            ...
            mDateTextPaint = Paint(Paint.ANTI_ALIAS_FLAG).apply {
                ...
                mDateTextSize = if (EventsCalendarUtil.dateTextFontSize == 0f) mContext.resources.getDimension(R.dimen.text_calendar_date)
                    ...
                    EventsCalendarUtil.dateTextFontSize, resources.displayMetrics)
            ...
            if (isCurrentMonth && !isDisabled) {
                ...
                when (EventsCalendarUtil.SELECTION_MODE) {
                    ...
                    EventsCalendarUtil.RANGE_SELECTION -> {
                        when {
                            EventsCalendarUtil.datesInSelectedRange.contains(
                                EventsCalendarUtil.getDateString(
                                    mDate,
                                    EventsCalendarUtil.DD_MM_YYYY
                                )
                            ) -> when {
                                ...
                                ) == EventsCalendarUtil.datesInSelectedRange.size - 1 -> {
                                    mDateTextPaint.isFakeBoldText =
                                        EventsCalendarUtil.isBoldTextOnSelectionEnabled
                                    mDateTextPaint.color = selectedTextColor
                    ...
                    EventsCalendarUtil.MULTIPLE_SELECTION -> {
                        if (EventsCalendarUtil.datesInSelectedRange.contains(
                                EventsCalendarUtil.getDateString(
                                    mDate,
                                    EventsCalendarUtil.DD_MM_YYYY
                                )
    ...
    private fun drawDot(canvas: Canvas) {
        if (isCurrentMonth && !isDisabled) {
            if (isSelected || EventsCalendarUtil.datesInSelectedRange.contains(
                    EventsCalendarUtil.getDateString(
                        mDate,
                        EventsCalendarUtil.DD_MM_YYYY
                    )
