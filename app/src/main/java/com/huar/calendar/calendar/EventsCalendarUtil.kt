...
object EventsCalendarUtil {
    ...
    fun updateSelectedDates(c: Calendar) {
        if (datesInSelectedRange.contains(getDateString(c,
                DD_MM_YYYY))
    ...
    fun updateMinMaxDateInRange(c: Calendar) {
        when {
            ...
            c.before(minDateInRange) || areDatesSame(c,
                minDateInRange) || minDateInRange == null -> {
                minDateInRange = c
                maxDateInRange = null
            }
    ...
    fun areDatesSame(date1: Calendar?, date2: Calendar?): Boolean =
        date1 != null && date2 != null && date1.get(Calendar.YEAR) == date2.get(Calendar.YEAR) && date1.get(
            Calendar.MONTH) == date2.get(Calendar.MONTH) && date1.get(Calendar.DAY_OF_MONTH) == date2.get(
            Calendar.DAY_OF_MONTH)

    fun areMonthsSame(date1: Calendar?, date2: Calendar?): Boolean =
        date1 != null && date2 != null && date1.get(Calendar.YEAR) == date2.get(Calendar.YEAR) && date1.get(
            Calendar.MONTH) == date2.get(Calendar.MONTH)
    ...
    fun getWeekCount(startDay: Calendar, endDay: Calendar): Int {
        var noOfWeeks = 0
        var finished = false
        val helper = MonthDisplayHelper(startDay.get(Calendar.YEAR),
    ...
    fun getMonthForWeekPosition(startMonth: Calendar, position: Int): Calendar {
        val helper = MonthDisplayHelper(startMonth.get(Calendar.YEAR),
    ...
    fun getCalendar(dateStr: String, format: Int): Calendar {
        ...
        when (format) {
            ...
            MM_DD_YYYY -> {
                ...
                date = Integer.parseInt(dateStr.substring(dateStr.indexOf('/') + 1,
                    dateStr.lastIndexOf('/')))
    ...
    fun getAssignCalendar(str: String, format: String?): Calendar {
        val calendar = Calendar.getInstance()
        var f = "yyyy-MM-dd"
        ...
        try {
            val dateFormat = SimpleDateFormat(f)
